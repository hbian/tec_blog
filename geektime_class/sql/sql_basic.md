...
* 视图vs临时表：视图是虚拟表，不存储数据。临时表是真实存在的数据表，不过它不用于长期存放数据，只为当前连接存在，关闭连接后，临时表就会自动释放。
* 视图的查询效率并不高

## 数据清洗
数据清洗的四个原则：
* 完整性: 对某一行来说是否存在空值，字段是否完善
* 全面性：观察某一列的全部数值以及特征，通过常识判断数据是否存在问题，比如日期格式是否统一
* 合法性：数据内容和数值是否合法，比如年龄>200, 性别未知等
* 唯一性：数据是否存在重复记录，行列都不能存在重复

三种常见的填充数据缺失的方式，分别是删除、均值和高频的方式：
* 删除：删除数据缺失的记录；
* 均值：使用当前列的均值；
* 高频：使用当前列出现频率最高的数据。
